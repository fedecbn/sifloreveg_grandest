...
class GridService {
    ...
    public function observation_fr5_ss_inf($cd_ref, $id_obj) {
        ...
        $fields = '';
        $columns = array();
        foreach ($this->grids['observations_veg_grid'] as $key => $values) {
        ...
        select " . $fields . " from exploitation_veg.obs_maille_fr5 obs
    ...
    public function observation_fr5($cd_ref, $id_obj) {
        ...
        select " . $fields . " from exploitation_veg.obs_maille_fr5 obs, exploitation_veg.taxref_v5_new tax
    ...
    public function observation_fr10_ss_inf($cd_ref, $id_obj) {
        ...
        select " . $fields . " from exploitation_veg.obs_maille_fr10 obs
	WHERE obs.cd_ref= :cd_ref and cd_sig= :id_obj ;");
        $statement->bindParam(':cd_ref', $cd_ref);
        $statement->bindParam(':id_obj', $id_obj);
        if ($statement->execute()) {
...
select " . $fields . " from exploitation_veg.obs_maille_fr10 obs, exploitation_veg.taxref_v5_new tax
    ...
    public function observation_comm_ss_inf($cd_ref, $id_obj) {
        ...
        select " . $fields . "  from exploitation_veg.obs_commune obs
    ...
    public function observation_comm($cd_ref, $id_obj) {
        ...
        select " . $fields . " from exploitation_veg.obs_commune obs, exploitation_veg.taxref_v5_new tax
    ...
    public function synthese_fr10_table($cd_ref, $offset, $ordField, $ordDir) {
        ...
        $fields = '';
        $columns = array();
        foreach ($this->grids['mailles_veg_grid'] as $key => $values) {
...
SELECT " . $fields . " FROM exploitation_veg.obs_maille_fr10
    WHERE cd_ref= :cd_ref
...
//TAXONS
    ...
    public function synthese_taxon_fr10($cd_ref) {
        ...
        $fields = '';
        $columns = array();
        foreach ($this->grids['taxons_veg_grid'] as $key => $values) {
...
SELECT  " . $fields . "
FROM exploitation_veg.synthese_syntaxon_fr10 syn, (WITH RECURSIVE hierarchie(cd_ref, nom_complet, cd_taxsup) AS (
  SELECT cd_ref::text, nom_complet, cd_taxsup::text
    FROM observation.taxref_v5 WHERE cd_ref = :cd_ref
  UNION ALL
  SELECT e.cd_ref::text, e.nom_complet, e.cd_taxsup::text
    FROM hierarchie AS h, observation.taxref_v5 AS e 
    WHERE h.cd_ref = e.cd_taxsup::text
)
SELECT cd_ref, nom_complet FROM hierarchie) recur
    ...
    SELECT   " . $fields . "
    FROM exploitation_veg.synthese_syntaxon_fr10 syn
    WHERE cd_ref = :cd_ref ;");
        $statement->bindParam(':cd_ref', $cd_ref);
        if ($statement->execute()) {
            while ($row = $statement->fetch()) {
...
SELECT " . $fields . "
FROM exploitation_veg.synthese_syntaxon_fr5 syn, (WITH RECURSIVE hierarchie(cd_ref, nom_complet, cd_taxsup) AS (
  SELECT cd_ref::text, nom_complet, cd_taxsup::text
    FROM observation.taxref_v5 WHERE cd_ref = :cd_ref
  UNION ALL
  SELECT e.cd_ref::text, e.nom_complet, e.cd_taxsup::text
...
SELECT " . $fields . "
FROM exploitation_veg.synthese_syntaxon_fr5 syn
    WHERE cd_ref = :cd_ref ;");
        $statement->bindParam(':cd_ref', $cd_ref);
        if ($statement->execute()) {
            while ($row = $statement->fetch()) {
...
SELECT " . $fields . "
    FROM exploitation_veg.synthese_syntaxon_comm syn, (WITH RECURSIVE hierarchie(cd_ref, nom_complet, cd_taxsup) AS (
...
SELECT " . $fields . "
    FROM exploitation_veg.synthese_syntaxon_comm syn
    WHERE cd_ref = :cd_ref;");
        $statement->bindParam(':cd_ref', $cd_ref);
        if ($statement->execute()) {
            while ($row = $statement->fetch()) {

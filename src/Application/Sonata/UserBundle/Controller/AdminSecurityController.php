...
class AdminSecurityController extends BaseController
{
    ...
    public function loginAction()
    {
        ...
        if($request->server->get('HTTP_REFERER')){
            if($request->server->get('HTTP_REFERER')!=$this->container->get('router')->generate('sonata_user_admin_security_login',array(),true)){
                $referer= $request->server->get('HTTP_REFERER');
            }else{
                $referer= $session->get('refTarget');
            }
        }else{ 
            $referer= $session->get('_security.main.target_path');   
        }
        $session->set('refTarget',$referer);
